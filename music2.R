...
query <- function(artists) {
    ...
    artist <- if (!is.null(raw$error)) {c("Not found")} else {raw$toptags$`@attr`$artist}
    genre <- if (!is.null(raw$error) || is.null(dim(raw$toptags$tag))) {c("Not found")} else {raw$toptags$tag$name[1]}
...
write.csv2(genre, "genres.csv", row.names = FALSE)

# Plot Top 10 genres by year
ranking <- music %>%
    filter(year(uts) >= 2015) %>%
...
ggplot(top_n(ranking, -10), aes(Year, rank)) +
    #geom_line(aes(color = genre, group = factor(genre)), size = 2) +
    geom_bump(aes(group = factor(genre), color = genre), size = 2) +
    ...
    geom_text(data = filter(label, Year == max(Year), min(Year) != max(Year)),
         ...
         hjust = 0) +
    scale_x_continuous(expand = c(0, 0.4, 0, 1), breaks = seq(2015, 2023, 1)) +
...
bar <- ggplot(music,  aes(year(uts))) +
    ...
    stat_bin(binwidth = 1,  geom = "text",  aes(label = format(..count..,  big.mark = ".")),  colour = "black",  drop = TRUE,  angle = 90,  hjust = -.1 )+
    labs(y = "# cançons") +
